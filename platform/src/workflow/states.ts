...
// 状态流转：
// draft ──提交──▶ in_review ──通过──▶ approved ──官方发布──▶ published
//    ▲              │              │
//    └── 打回 ◀─────┘              └──人工发布包──▶ ready_to_publish ──确认发布──▶ published
//                   ▲                                      │
//                   └────────────── 退回重审 ◀──────────────┘
//
...
/** 渠道稿的协作状态。 */
export type Status = 'draft' | 'in_review' | 'approved' | 'ready_to_publish' | 'published'

/**
 * 合法流转表：键为「当前状态」，值为「可直接转入的目标状态」列表。
 * - draft      → in_review                （提交送审）
 * - in_review  → approved | draft          （通过 / 打回，打回时带 reason）
 * - approved   → published | ready_to_publish | in_review
 *                                      （官方发布 / 人工发布包已准备 / 退回重审）
 * - ready_to_publish → published | in_review（人工确认已发 / 退回重审）
 * - published  → （终态，不可再流转）
 */
export const TRANSITIONS: Record<Status, Status[]> = {
  ...
  approved: ['published', 'ready_to_publish', 'in_review'],
  ready_to_publish: ['published', 'in_review'],
  published: [],
}
